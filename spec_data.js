...
# REAL_ESTATE_SYSTEM_SPEC (v2.6 - 2026-07-30 운영안정화판 / 수정기능·작업로그·정합성 반영본)
...
### 2026-07-29~30 (v2.5/v2.6 운영 안정화)

[수정기능] 전 화면에 [✏️ 수정] 버튼 노출 (부동산·계약서·계약자·공과금·월세납부·유지보수·팀원·협력사).
  신규 수정 API: rooms/buildings/bills/incidents/payments — 모두 수정 전 자동 스냅샷(되돌리기 가능).

[작업기록] audit_logs 재정비 + GET /api/v1/auditlogs(?date=YYYY-MM-DD | dates).
  guard.js가 모든 POST에 로그인 사용자(_actor) 자동 첨부. 팀원관리 우측 [📜 작업 기록] 탭에서 날짜별 조회.
  ※ 로그는 삭제하지 않고 날짜별 누적 보관.

[정합성 버그 수정]
 · 계약↔계약자 미연동 → _resolve_tenant()로 contacts(tenant) 생성/연결(tenant_contact_id). 중복 계약자 생성 없음.
 · 호실 중복 등록 → 409 차단 + 해당 호실 폼 자동 로드.
 · 한 호실 계약 중복 → 진행중 계약 있으면 409 차단 + '불러와 수정' 유도(만료 후 신규 허용).
 · 공실 오집계 → 계약 시 _mark_room_leased()로 '임대' 자동 전환, 금일현황은 유효계약 기준 판정.
 · 호실 상태 3분류 확정: 공실/임대/공사중 (기존값 9건 '임대'로 마이그레이션, 편집 시 기존값 보존).

[연동 정상화]
 · 통합대시보드: 부가세 무조건 10% → 계약 vat_applied 반영('면제' 표기). 가짜 공과금·가짜 전화번호 제거 → 실제 bills/임차인 연락처.
 · 월세납부: 우측 표 '항상 완납' 하드코딩 → 당월 payments 기준 실제 상태. 공과금 자동값 전부 0, 호실 선택 시 검침 bills 자동 반영.
   정산유형에 미납(연체료 면제)·미납 추가, 선택 시 버튼 🔴 미납 기록 저장으로 전환.

[UI 통일] 호실 선택 드롭다운(74개) 전면 제거 → 번호 입력 + 같은 층 버튼(440→4층, B1→지하1층).
  계약서·공과금(좌우 연동, 우측 검색창 신설/'앞10개 고정' 버그 제거)·월세납부·유지보수·부동산(좌측 검색 신설).

[금일현황] 프린트 최적화 — 카드/폰트 축소, 오늘의 연락 리스트 삭제,
  공실 목록을 건물별+층별 한 줄로 압축(65줄→7줄), 지하 표기·최상단 정렬, 면적/상태 열 제거.

[기타] 공용부(scope/building_id/common_area) 등록, 관리자 비번 재변경(sha256), 직원 추가(121 손승연),
  build_windows.bat 한글파일명 오류 수정(copy *.txt), 노션에 터미널 명령어/폴더구조 메모 작성.
